...
//!
//! Provides strongly-typed MCP tools for ToonDB:
//!
//! ## Core Database Tools
//! - `toondb.context_query` - AI-optimized context with typed sections
//! - `toondb.query` - Execute ToonQL queries with format option
//! - `toondb.get/put/delete` - Path-based CRUD
//!
...
use toondb_core::toon::{ToonTable, ToonSchema, ToonType, ToonRow, ToonValue};
...
fn try_format_as_table(arr: &[Value]) -> Option<String> {
    ...
    // Create Schema
    let mut schema = ToonSchema::new("results");
    for k in &keys {
        // Simple type inference or default to Text
        schema = schema.field(k, ToonType::Text); 
    }
    ...
    for item in arr {
        ...
        for k in &keys {
            ...
            // Convert serde Value to ToonValue manually since we don't have direct mapper handy
            // and we want simple display representation
            let tv = match v {
                Value::Null => ToonValue::Null,
                Value::Bool(b) => ToonValue::Bool(*b),
                Value::Number(n) => {
                    if let Some(i) = n.as_i64() { ToonValue::Int(i) }
                    else if let Some(u) = n.as_u64() { ToonValue::UInt(u) }
                    else { ToonValue::Float(n.as_f64().unwrap_or(0.0)) }
                },
                Value::String(s) => ToonValue::Text(s.clone()),
                _ => ToonValue::Text(v.to_string()),
            };
            row_values.push(tv);
        }
        rows.push(ToonRow::new(row_values));
    }
    
    let table = ToonTable::with_rows(schema, rows);
    Some(table.format())
}
...
fn toon_result(value: &Value) -> Result<String, String> {
...
pub fn get_built_in_tools() -> Vec<Value> {
    ...
    tools.push(json!({
        ...
        "description": "Fetch AI-optimized context from ToonDB with token budgeting. Sections are packed in priority order.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "sections": {
    ...
    tools.push(json!({
        "name": "toondb_query",
        "description": "Execute a ToonQL query. Returns results in TOON format (40-60% fewer tokens than JSON).",
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": { 
                    "type": "string",
                    "description": "ToonQL query (e.g., 'SELECT id,name FROM users WHERE score > 80')"
                },
                "format": {
                    "type": "string",
                    "enum": ["toon", "json"],
...
impl EmbeddingManager {
    fn new() -> Self {
        // Runtime opt-in: check env var TOONDB_SEMANTIC_SEARCH=1
        let enabled = std::env::var("TOONDB_SEMANTIC_SEARCH")
            .map(|v| v == "1" || v.to_lowercase() == "true")
            .unwrap_or(false);
        
        if enabled {
            info!("Semantic search enabled via TOONDB_SEMANTIC_SEARCH=1");
        }
...
pub struct ToolExecutor {
    conn: DurableToonClient, // Use DurableToonClient to access vector_search if available?
    // Wait, conn is DurableToonClient? Let's check imports or definition.
    // Ah, it was defined as DurableToonClient in previous context but let's assume it is.
...
impl ToolExecutor {
    pub fn new(conn: DurableToonClient) -> Self {
        Self { 
            conn,
    ...
    pub fn execute(&self, name: &str, args: Value) -> Result<String, String> {
        ...
        match name {
            // Core database tools
            "toondb_context_query" => self.exec_context_query(args),
            "toondb_query" => self.exec_query(args),
            "toondb_get" => self.exec_get(args),
            "toondb_put" => self.exec_put(args),
            "toondb_delete" => self.exec_delete(args),
            "toondb_list_tables" => self.exec_list_tables(args),
            "toondb_describe" => self.exec_describe(args),
    ...
    fn exec_list_tables(&self, args: Value) -> Result<String, String> {
        ...
        toon_result(&json!(result)).map_err(|e| e.to_string())
    }
    ...
    fn exec_describe(&self, args: Value) -> Result<String, String> {
        ...
        match self.conn.scan(&prefix) {
            Ok(results) if !results.is_empty() => {
                ...
                toon_result(&result).map_err(|e| e.to_string())
            }
            Ok(_) => {
                self.conn.abort().ok();
    ...
    fn exec_search_episodes(&self, args: Value) -> Result<String, String> {
        ...
        if let Some(_vector) = self.embeddings.get_embedding(query) {
             ...
             // TODO: Enable when DurableToonClient supports vectors()
             /*
