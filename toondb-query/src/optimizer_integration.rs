...
//!
//! Wires the QueryOptimizer into the TOON-QL execution path for cost-based planning:
//! - Converts TOON-QL WHERE clauses to optimizer predicates
...
#[cfg(test)]
use crate::toon_ql::{ComparisonOp, WhereClause};
use crate::toon_ql::{SelectQuery, ToonResult, ToonValue};
use parking_lot::RwLock;
use std::collections::HashMap;
use std::sync::Arc;
use toondb_core::{Catalog, Result};
use toondb_storage::sketches::HyperLogLog;
...
pub trait StorageBackend: Send + Sync {
    ...
    ) -> Result<Vec<HashMap<String, ToonValue>>>;
    ...
    fn vector_search(
        ...
        query: &[f32],
        k: usize,
    ) -> Result<Vec<(f32, HashMap<String, ToonValue>)>>;
...
impl OptimizedExecutor {
    ...
    fn condition_to_predicate(&self, column: &str, value: &ToonValue) -> Option<QueryPredicate> {
        // Detect special column patterns
        match column {
            // Time-based columns
            "timestamp" | "created_at" | "updated_at" | "time" => {
                if let ToonValue::UInt(ts) = value {
            ...
            // Project ID
            "project_id" | "project" => {
                if let ToonValue::UInt(id) = value {
    ...
    fn build_execution_plan(
        ...
        if let Some(order_by) = &select.order_by {
            steps.push(ExecutionStep::Sort {
                ...
                ascending: order_by.direction == crate::toon_ql::SortDirection::Asc,
            });
        }
    ...
    pub fn execute<S: StorageBackend>(
        ...
        for step in &plan.execution_plan.steps {
            match step {
                ...
                ExecutionStep::MultiIndexIntersect { table, indexes } => {
                    ...
                    let mut result_sets: Vec<Vec<HashMap<String, ToonValue>>> = Vec::new();
        ...
        let result_rows: Vec<Vec<ToonValue>> = rows
            .iter()
            .map(|row| {
                columns_to_return
                    .iter()
                    .map(|c| row.get(c).cloned().unwrap_or(ToonValue::Null))
                    .collect()
            })
            .collect();
    ...
    fn extract_primary_key_from_predicates(
        ...
        for pred in predicates {
            ...
            if let QueryPredicate::Project(id) = pred {
                return Some(ToonValue::UInt(*id as u64));
            }
        }
        None
    }
    ...
    fn extract_index_key_from_predicates(
        ...
        for pred in predicates {
            match pred {
                QueryPredicate::Tenant(id) => return Some(ToonValue::UInt(*id as u64)),
    ...
    fn intersect_result_sets(
        &self,
        sets: Vec<Vec<HashMap<String, ToonValue>>>,
    ) -> Vec<HashMap<String, ToonValue>> {
        if sets.is_empty() {
            return Vec::new();
        }
        if sets.len() == 1 {
    ...
    /// Compare ToonValue for sorting
    fn compare_values(a: Option<&ToonValue>, b: Option<&ToonValue>) -> std::cmp::Ordering {
        match (a, b) {
            (None, None) => std::cmp::Ordering::Equal,
            (None, Some(_)) => std::cmp::Ordering::Less,
            (Some(_), None) => std::cmp::Ordering::Greater,
            (Some(va), Some(vb)) => match (va, vb) {
                (ToonValue::Int(a), ToonValue::Int(b)) => a.cmp(b),
                (ToonValue::UInt(a), ToonValue::UInt(b)) => a.cmp(b),
                (ToonValue::Float(a), ToonValue::Float(b)) => {
                    a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal)
                }
                (ToonValue::Text(a), ToonValue::Text(b)) => a.cmp(b),
                (ToonValue::Bool(a), ToonValue::Bool(b)) => a.cmp(b),
...
mod tests {
    use super::*;
    use crate::toon_ql::{Condition, LogicalOp, OrderBy, SortDirection};
        ...
        let select = SelectQuery {
            ...
            where_clause: Some(WhereClause {
                conditions: vec![Condition {
                    ...
                    value: ToonValue::UInt(1700000000000000),
                }],
