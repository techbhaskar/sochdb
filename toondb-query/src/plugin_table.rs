...
use crate::toon_ql::{SelectQuery, ToonResult, ToonValue, WhereClause};
...
pub trait VirtualTable: Send + Sync {
    ...
    fn get(&self, key: &ToonValue) -> Result<Option<VirtualRow>, VirtualTableError> {
...
impl VirtualRow {
    ...
    /// Get value by index
    pub fn get(&self, idx: usize) -> Option<&ToonValue> {
        self.values.get(idx)
    }
...
pub enum VirtualFilter {
    ...
    In(String, Vec<ToonValue>),
    /// Between: column BETWEEN low AND high
    Between(String, ToonValue, ToonValue),
...
impl VirtualFilter {
    ...
    pub fn from_where_clause(where_clause: &WhereClause) -> Self {
        ...
        match where_clause.operator {
            crate::toon_ql::LogicalOp::And => VirtualFilter::And(filters),
            crate::toon_ql::LogicalOp::Or => VirtualFilter::Or(filters),
        }
    }
    ...
    pub fn matches(&self, row: &VirtualRow, schema: &VirtualTableSchema) -> bool {
        match self {
            ...
            VirtualFilter::Like(col, pattern) => row
                .get_by_name(col, schema)
                .and_then(|v| match v {
                    ToonValue::Text(s) => Some(Self::match_like(s, pattern)),
                    _ => None,
                })
                .unwrap_or(false),
    ...
    fn compare_values<F>(val: Option<&ToonValue>, other: &ToonValue, cmp: F) -> bool
    where
        F: Fn(i64, i64) -> bool,
    {
        match (val, other) {
            (Some(ToonValue::Int(a)), ToonValue::Int(b)) => cmp(*a, *b),
            (Some(ToonValue::UInt(a)), ToonValue::UInt(b)) => cmp(*a as i64, *b as i64),
            (Some(ToonValue::Float(a)), ToonValue::Float(b)) => {
...
impl PluginVirtualTable {
    ...
    fn project_columns(&self, rows: &[VirtualRow], columns: &[String]) -> Vec<VirtualRow> {
        ...
        rows.iter()
            .map(|row| {
                let values: Vec<ToonValue> = indices
                    .iter()
                    .map(|&i| row.values.get(i).cloned().unwrap_or(ToonValue::Null))
                    .collect();
    ...
    fn generate_mock_data(&self, count: usize) -> Vec<VirtualRow> {
        (0..count)
            .map(|i| {
                let values: Vec<ToonValue> = self
                    .schema
                    .columns
                    .iter()
                    .enumerate()
                    .map(|(col_idx, col)| match col.col_type {
                        VirtualColumnType::Int64 => ToonValue::Int(i as i64 + col_idx as i64),
                        VirtualColumnType::UInt64 => ToonValue::UInt(i as u64 + col_idx as u64),
                        VirtualColumnType::Float64 => ToonValue::Float(i as f64 * 0.1),
                        VirtualColumnType::Text => ToonValue::Text(format!("{}_{}", col.name, i)),
                        VirtualColumnType::Bool => ToonValue::Bool(i % 2 == 0),
                        _ => ToonValue::Null,
                    })
                    .collect();
                VirtualRow::new(values)
...
impl VirtualTableRegistry {
    ...
    pub fn execute_select(&self, query: &SelectQuery) -> Result<ToonResult, VirtualTableError> {
        let table = self
            .get(&query.table)
        ...
        let result_rows: Vec<Vec<ToonValue>> = rows.into_iter().map(|r| r.values).collect();
...
mod tests {
    ...
    fn test_virtual_table_scan_with_filter() {
        ...
        let filter = VirtualFilter::Gt("id".to_string(), ToonValue::Int(5));
        let rows = table.scan(&[], Some(&filter), Some(100), None).unwrap();

        // All rows should have id > 5
        for row in &rows {
            if let Some(ToonValue::Int(id)) = row.get_by_name("id", &schema) {
                assert!(*id > 5);
            }
        }
    }
    ...
    fn test_virtual_filter_matches() {
        ...
        let row = VirtualRow::new(vec![
            ToonValue::Int(42),
            ToonValue::Text("Alice".to_string()),
            ToonValue::Float(95.5),
        ]);

        // Equality filter
        let filter = VirtualFilter::Eq("id".to_string(), ToonValue::Int(42));
        ...
        // Greater than
        let filter = VirtualFilter::Gt("score".to_string(), ToonValue::Float(90.0));
        assert!(filter.matches(&row, &schema));

        // AND filter
        let filter = VirtualFilter::And(vec![
            VirtualFilter::Eq("id".to_string(), ToonValue::Int(42)),
            VirtualFilter::Gt("score".to_string(), ToonValue::Float(90.0)),
        ]);
        assert!(filter.matches(&row, &schema));
    }
