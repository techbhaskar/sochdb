...
//! ToonDB Database Kernel
//!
//! The shared core that powers both embedded mode (`ToonConnection::open`) and
//! server mode (`toondb-server`). This is the "SQLite engine" equivalent.
//!
//! ## Architecture
//!
//! ```text
...
use toondb_core::{Result, ToonDBError, ToonValue};
...
fn format_toon_value(v: &ToonValue) -> String {
    match v {
        ToonValue::Null => "∅".to_string(),
        ToonValue::Int(i) => i.to_string(),
        ToonValue::UInt(u) => u.to_string(),
        ToonValue::Float(f) => format!("{:.6}", f),
        ToonValue::Text(s) => {
        ...
        ToonValue::Bool(b) => if *b { "T" } else { "F" }.to_string(),
        ToonValue::Binary(b) => format!("b64:{}", base64_encode(b)),
        _ => format!("{:?}", v),
    }
}
...
use toondb_core::TypedColumn as CoreTypedColumn;

/// Columnar query result - SIMD-friendly format for analytics
///
/// Instead of row-oriented `Vec<HashMap<String, ToonValue>>`, this returns
...
impl ColumnarQueryResult {
    ...
    pub fn column_stats(&self, column: &str) -> Option<&toondb_core::columnar::ColumnStats> {
...
pub struct VectorSearchResult {
    pub id: u64,
    pub distance: f32,
    pub metadata: Option<HashMap<String, ToonValue>>,
}

/// The ToonDB Database Kernel
///
/// This is the shared core used by both embedded (`ToonConnection`) and
/// server (`toondb-server`) modes. It owns all storage, catalog, and
/// indexing components.
///
/// # Thread Safety
///
...
impl Database {
    ...
    // =========================================================================
    // Path-Native API (ToonDB's differentiator)
    // =========================================================================
    ...
    pub fn insert_row(
        ...
        table: &str,
        row_id: u64,
        values: &HashMap<String, ToonValue>,
    ) -> Result<()> {
        // Use cached packed schema - single DashMap lookup, no clone
        let packed_schema = self
            .packed_schemas
            .get(table)
            .ok_or_else(|| ToonDBError::InvalidArgument(format!("Table '{}' not found", table)))?;
    ...
    pub fn read_row(
        ...
        row_id: u64,
        columns: Option<&[&str]>,
    ) -> Result<Option<HashMap<String, ToonValue>>> {
        let schema = self
            .tables
            .get(table)
            .ok_or_else(|| ToonDBError::InvalidArgument(format!("Table '{}' not found", table)))?;
        ...
        let packed_schema = self
            .packed_schemas
            .get(table)
            .ok_or_else(|| ToonDBError::Internal("Packed schema not found".into()))?;
    ...
    pub fn insert_rows_batch(
        ...
        txn: TxnHandle,
        table: &str,
        rows: &[(u64, HashMap<String, ToonValue>)],
    ) -> Result<usize> {
        // Use cached packed schema
        let packed_schema = self
            .packed_schemas
...
impl<'a> QueryBuilder<'a> {
    ...
    pub fn execute(self) -> Result<QueryResult> {
        ...
        let mut rows: Vec<HashMap<String, ToonValue>> = Vec::new();
        ...
        if let Some(ref schema) = schema {
            ...
            let mut rows_map: HashMap<String, HashMap<String, ToonValue>> = HashMap::new();
    ...
    ///
    /// Returns an iterator over rows as `Vec<ToonValue>` in schema column order.
    ...
    ///     let row = row_result?;
    ///     // row is Vec<ToonValue> in column order
    /// }
    /// ```
    ...
    ///
    /// Instead of row-oriented `Vec<HashMap<String, ToonValue>>`, returns
...
impl Iterator for QueryRowIterator {
    type Item = Result<Vec<ToonValue>>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            ...
            if let Some(ref schema) = self.packed_schema {
                ...
                self.yielded += 1;
                return Some(Ok(vec![ToonValue::Binary(value_bytes)]));
            }
        }
    }
}
...
#[allow(dead_code)]
fn serialize_value(value: &ToonValue) -> Vec<u8> {
    // Simple serialization - in production use proper format
    match value {
        ToonValue::Null => vec![0],
        ToonValue::Int(i) => {
            let mut buf = vec![1];
        ...
        ToonValue::Bool(b) => vec![5, if *b { 1 } else { 0 }],
        ToonValue::Binary(b) => {
            let mut buf = vec![6];
...
fn deserialize_value(bytes: &[u8]) -> ToonValue {
    if bytes.is_empty() {
    ...
    match bytes[0] {
        ...
        5 if bytes.len() >= 2 => ToonValue::Bool(bytes[1] != 0),
        6 => ToonValue::Binary(bytes[1..].to_vec()),
        _ => {
            // Treat as text
...
/// Push a ToonValue into a TypedColumn
fn push_value_to_typed_column(col: &mut CoreTypedColumn, value: Option<ToonValue>) {
    match value {
...
mod tests {
    ...
    fn test_database_table_api() {
        ...
        values.insert("name".to_string(), ToonValue::Text("Alice".to_string()));
        values.insert("age".to_string(), ToonValue::Int(30));
        ...
        let row = row.unwrap();
        assert_eq!(row.get("name"), Some(&ToonValue::Text("Alice".to_string())));
